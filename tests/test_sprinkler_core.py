...
class TestSprinklerCore(SprinklerBasetest):
    ...
    def test_spray_points_calculation(self):
        ...
        for h_angle, v_angle, end_point in spray_points:
            ...
            distance = ((end_point.x - start[0])**2 +
                        (end_point.y - start[1])**2 +
                        (end_point.z - start[2])**2)**0.5
    ...
    def test_is_point_within_boundaries(self):
        ...
        self.assertTrue(self.system.is_point_within_boundaries(Point3D(self.config.lawn.width - 0.1, self.config.lawn.length - 0.1, 0.01)))

        # Test points outside the lawn
        self.assertFalse(self.system.is_point_within_boundaries(Point3D(-0.1, -0.1, 0.01)))
        self.assertFalse(self.system.is_point_within_boundaries(Point3D(self.config.lawn.width + 0.1, self.config.lawn.length + 0.1, 0.01)))

        # Test points above max height
        self.assertFalse(self.system.is_point_within_boundaries(Point3D(0.1, 0.1, 20)))  # Assuming max height is less than 20m
