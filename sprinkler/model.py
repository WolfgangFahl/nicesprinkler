...
class SprinklerModel:
    ...
    def calculate_spray_points(self) -> List[Tuple[float, float, float]]:
        ...
        for h_angle in range(-90, 91, 5):
            for v_angle in range(10, 61, 5):
                ...
                spray_height = sprinkler_pos["z"] + distance * math.tan(
    ...
    def calculate_spray_distance(self, angle: float) -> float:
        v0 = math.sqrt(
            ...
            * 9.8
            / math.sin(
                2 * math.radians(self.config["hose_performance"]["optimal_angle"])
            )
        )
    ...
    def is_point_within_boundaries(
        ...
        if (
            x < 0
            or x > self.config["lawn"]["width"]
            or y < 0
            or y > self.config["lawn"]["length"]
        ):
            return False
